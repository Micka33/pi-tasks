...
import { DEFAULT_CLAIM_TTL_SECONDS, SCOPE_TYPES, TASK_STATUSES, VISIBILITIES, } from "./types.js";
export class TaskService {
    ...
    createTaskList(input, access) {
        ...
        return withImmediateTransaction(this.db, () => {
            this.db
                .prepare(`INSERT INTO task_lists
    ...
    findTaskLists(input, access) {
        ...
        const rows = this.db.prepare(`SELECT * FROM task_lists ${where} ORDER BY updated_at DESC, created_at DESC`).all(...params);
        ...
        for (const list of lists) {
            ...
            if (!input.include_inaccessible_private)
                continue;
    ...
    createTask(input, access) {
        ...
        return withImmediateTransaction(this.db, () => {
           ...
           VALUES (?, ?, ?, ?, ?, ?, 'todo', ?, NULL, NULL, NULL, ?, ?, NULL, NULL, NULL)`)
                .run(id, input.list_id, position, input.title.trim(), normalizeNullableString(input.description), normalizeNullableString(input.notes), normalizeNullableString(input.assigned_to_agent_id), now, now);
            return this.getTaskRow(id);
        });
    }
    addManyTasks(input, access) {
        ...
        return withImmediateTransaction(this.db, () => {
            ...
            const created = [];
            const insert = this.db.prepare(`INSERT INTO tasks
          (id, list_id, position, title, description, notes, status, assigned_to_agent_id, claimed_by_agent_id,
           claim_expires_at, outcome, created_at, updated_at, started_at, completed_at, deleted_at)
         VALUES (?, ?, ?, ?, ?, ?, 'todo', ?, NULL, NULL, NULL, ?, ?, NULL, NULL, NULL)`);
            for (const item of input.tasks) {
                ...
                position += 1;
                insert.run(id, input.list_id, position, item.title.trim(), normalizeNullableString(item.description), normalizeNullableString(item.notes), normalizeNullableString(item.assigned_to_agent_id), now, now);
                created.push(this.getTaskRow(id));
            }
            return created;
        });
    }
    claimNextTask(input, access) {
        return withImmediateTransaction(this.db, () => {
            this.getTaskListForAccess(input.list_id, access);
            const now = this.nowIso();
           ...
           LIMIT 1`)
                .get(input.list_id, access.actor.agentId);
            if (!task)
                return { task: null };
    ...
    reorderTasks(input, access) {
        return withImmediateTransaction(this.db, () => {
            this.getTaskListForAccess(input.list_id, access);
            if (!Array.isArray(input.task_ids))
                throw new ValidationError("task_ids must be an array");
            ...
            for (const id of input.task_ids) {
                if (seen.has(id))
                    throw new ValidationError(`Duplicate task id in reorder: ${id}`);
                seen.add(id);
                const task = byId.get(id);
                if (!task)
                    throw new NotFoundError("active task in list", id);
                ordered.push(task);
            }
    ...
    releaseExpiredClaims(input, access) {
        return withImmediateTransaction(this.db, () => {
            const now = this.nowIso();
    ...
    releaseExpiredClaimsInternal(input, access, now) {
        ...
        const rows = this.db
            .prepare(`SELECT tasks.* FROM tasks
        ...
        const released = [];
        const update = this.db.prepare("UPDATE tasks SET status = 'todo', claimed_by_agent_id = NULL, claim_expires_at = NULL, updated_at = ? WHERE id = ?");
    ...
    getTaskListForAccess(listId, access, options = {}) {
        const list = this.getTaskListRow(listId, { includeDeleted: options.includeDeleted });
        this.assertListAccess(list, access);
        return list;
    }
    getTaskListRow(listId, options = {}) {
        const row = this.db.prepare("SELECT * FROM task_lists WHERE id = ?").get(listId);
        if (!row)
            throw new NotFoundError("task_list", listId);
        const list = rowToTaskList(row);
        if (list.deleted_at && !options.includeDeleted)
            throw new NotFoundError("task_list", listId);
        return list;
    }
    getTaskRow(taskId) {
    ...
    getTasksForList(listId, options = {}) {
    ...
    nextPosition(listId, requestedPosition) {
        if (requestedPosition === undefined)
            return this.maxActivePosition(listId) + 1;
    ...
    canAccessList(list, actor) {
        if (list.visibility === "shared")
            return true;
        if (list.owner_agent_id)
            return list.owner_agent_id === actor.agentId;
        return list.created_by_agent_id === actor.agentId;
    }
    assertListAccess(list, access) {
        if (this.canAccessList(list, access.actor))
            return;
...
function rowToTaskList(row) {
    return {
        ...
        scope_type: stringField(row, "scope_type"),
...
function rowToPrivateAccessEvent(row) {
    return {
        id: stringField(row, "id"),
        list_id: stringField(row, "list_id"),
...
function validateScopeType(value) {
    if (!SCOPE_TYPES.includes(value)) {
