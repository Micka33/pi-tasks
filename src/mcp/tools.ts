...
export function registerMcpTaskTools(server: McpServer): void {
  const definitions: McpTaskToolDefinition[] = [
    ...
    },
    {
      name: "task_update",
      title: "Update Task",
      description: "Update task fields, notes, outcome, or status. Notes are task-local working memory. Setting status to in_progress is rejected; use task_claim_next instead. Closing a task as done/canceled requires outcome with choices/decisions, actions taken, and result obtained. Setting status to blocked keeps assignment on the pausing agent unless assigned_to_agent_id:null is passed." + COMMON_DESCRIPTION_SUFFIX,
